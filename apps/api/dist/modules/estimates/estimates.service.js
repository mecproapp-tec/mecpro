...
let EstimatesService = EstimatesService_1 = class EstimatesService {
    ...
    async create(tenantId, data) {
        ...
        try {
            ...
            this.logger.error(`Erro ao criar orçamento`, error?.stack || error);
            throw new common_1.InternalServerErrorException('Erro ao criar orçamento');
        }
    }
    async findAll(tenantId, page = 1, limit = 50) {
        ...
        const where = {
            ...
            deletedAt: null,
            status: {
                not: client_1.EstimateStatus.CONVERTED,
            },
        };
    ...
    async update(id, tenantId, data) {
        ...
        if (estimate.status ===
            client_1.EstimateStatus.CONVERTED) {
    ...
    async convertToInvoice(estimateId, tenantId) {
        ...
        try {
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                if (existingInvoice) {
                    throw new common_1.ConflictException('Já existe fatura vinculada');
                }
            ...
            if (error?.code === 'P2002') {
                throw new common_1.ConflictException('Duplicação detectada');
            }
            throw new common_1.InternalServerErrorException('Erro ao converter orçamento');
        }
    }
    ...
    async generatePdfNow(estimate) {
        ...
        if (this.pdfGeneratingLocks.has(estimateId)) {
            let attempts = 0;
            while (this.pdfGeneratingLocks.has(estimateId) &&
                attempts < 30) {
            ...
            if (updatedEstimate?.pdfUrl) {
                return {
                    pdfUrl: updatedEstimate.pdfUrl,
                    pdfKey: updatedEstimate.pdfKey,
                };
            }
        }
        ...
        try {
            ...
            const pdfBuffer = await this.estimatesPdfService.generateEstimatePdf(payload);
            ...
            this.logger.error(`Erro ao gerar PDF`, error?.stack || error);
            await this.prisma.estimate
                ...
                .catch(() => null);
            throw new common_1.BadRequestException('Erro ao gerar PDF');
        }
        finally {
    ...
    async sendToWhatsApp(id, tenantId, phoneNumber) {
        ...
        const finalPhone = cleanPhone.startsWith('55')
            ? cleanPhone
            : `55${cleanPhone}`;
        const message = `📄 *ORÇAMENTO #${estimate.id}*\n` +
