...
export class EstimatesService {
  ...
  async create(tenantId: string, data: any) {
    ...
    try {
      ...
      this.logger.log(
        `✅ Orçamento criado com ID: ${estimate.id}`,
      );
  ...
  private async generatePdfNow(estimate: any) {
    ...
    // EVITA GERAÇÃO DUPLICADA
    if (this.pdfGeneratingLocks.has(estimateId)) {
      this.logger.log(
        `⏳ PDF do orçamento ${estimateId} já está sendo gerado`,
      );

      let attempts = 0;

      while (
        this.pdfGeneratingLocks.has(estimateId) &&
        attempts < 30
      ) {
    ...
    try {
      ...
      this.logger.log(
        `✅ PDF gerado para orçamento ${estimate.id}`,
      );
      ...
      this.logger.error(
        `❌ Erro ao gerar PDF ${estimate.id}: ${error.message}`,
      );
  ...
  private async ensurePdf(
    ...
    if (!estimate.pdfUrl || tenantChanged) {
      this.logger.log(
        `♻️ Regenerando PDF do orçamento ${estimate.id}`,
      );
    ...
    return await this.prisma.$transaction(
      async (tx) => {
        ...
        if (
          ...
          if (!client) {
            throw new BadRequestException(
              'Cliente não pertence ao tenant',
            );
          }
    ...
    while (retries < maxRetries) {
      try {
        return await this.prisma.$transaction(
          async (tx) => {
            ...
            if (
              ...
              throw new BadRequestException(
                'Status inválido para conversão',
              );
            }
            ...
            this.logger.log(
              `✅ Orçamento ${estimateId} convertido para ${invoice.number}`,
            );
        ...
        if (
          ...
          this.logger.warn(
            `⚠️ Tentativa ${retries}/${maxRetries}`,
          );
