...
export default function EditarAgendamento() {
  ...
  const carregarAgendamento = async () => {
    try {
      ...
      // Converte a string UTC para o fuso local (Brasília) para preencher os inputs
      const dataUTC = new Date(app.date);
      // Formata a data no padrão YYYY-MM-DD (usando en-CA)
      const localDate = dataUTC.toLocaleDateString('en-CA');
      // Formata a hora no padrão HH:MM (24h)
      const localTime = dataUTC.toLocaleTimeString('en-CA', { hour12: false, hour: '2-digit', minute: '2-digit' });
