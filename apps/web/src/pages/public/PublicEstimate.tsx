...
export default function PublicEstimate() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get(`/public/estimates/share/${token}`);
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { bg: string; text: string; label: string }> = {
      ...
      CONVERTED: { bg: 'bg-blue-100', text: 'text-blue-800', label: 'Convertido' },
    };
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-blue-600 text-white p-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            {data.tenant.logoUrl && <img src={data.tenant.logoUrl} alt="Logo" className="h-12 w-auto bg-white rounded p-1" />}
            <div>
              <h1 className="text-2xl font-bold">Orçamento #{data.id}</h1>
              <div className="flex items-center gap-2 mt-1">
                {getStatusBadge(data.status)}
                <span className="text-sm opacity-90">• {new Date(data.date).toLocaleDateString('pt-BR')}</span>
              </div>
        ...
        <div className="p-6">
          ...
          {data.pdfUrl && (
            <div className="mt-6 text-center">
              <a href={data.pdfUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">
