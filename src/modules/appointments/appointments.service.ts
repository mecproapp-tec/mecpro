...
export class AppointmentsService {
  ...
  private isCronRunning = false; // 🔒 Lock para evitar execuções concorrentes
  ...
  async create(tenantId: string, data: CreateAppointmentDto) {
    // 🔥 Log para depuração (ver se está sendo chamado múltiplas vezes)
  ...
  // ============================
  // Job automático (executa a cada minuto)
  // ============================
  ...
  async checkAppointmentsForNotifications() {
    ...
    try {
      ...
      for (const app of appointments) {
        ...
        // 🔥 Verifica se já existe notificação para este appointment (evita duplicata)
